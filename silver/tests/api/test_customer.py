...
class TestCustomerEndpoints(APITestCase):
    ...
    def test_create_post_customer_without_required_field(self):
        ...
        for field in required_fields:
            ...
            try:
                ...
                pytest.xfail(
                    "Customer required field %s not provided in the"
    ...
    def test_edit_put_customer(self):
        ...
        response = self.client.put(
            url, data=json.dumps(changed_data), content_type="application/json"
        )
        ...
        for field in response.data:
            if field not in unchanged_fields:
                self.assertEqual(response.data[field], self.complete_data[field])

    def test_edit_patch_customer(self):
        customer = CustomerFactory.create()

        changed_data = self.complete_data.copy()
        unchanged_fields = ["email", "zip_code", "company", "phone", "payment_due_days"]
        ignore_fields = [
            "url",
            "id",
            "subscriptions",
