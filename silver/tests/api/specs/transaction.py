...
from silver.tests.api.specs.url import (
    ...
    spec_provider_url,
    spec_proforma_url,
    spec_transaction_url,
    spec_transaction_pay_url,
    spec_payment_method_url,
)
...
transaction_definition = ResourceDefinition(
    "transaction",
    {
        "id": {
            "read_only": True,
        ...
        "customer": {
            ...
            "assertions": [lambda input, transaction, output: input == output],
        },
        "provider": {
            "expected_input_types": str,
        ...
        "amount": {
            ...
            "assertions": [
                lambda input, transaction, output: (input == output if input else True)
            ],
        },
        "state": {
            "read_only": True,
        ...
        "proforma": {
            "expected_input_types": str,
            "output": lambda transaction: (
                spec_proforma_url(transaction.proforma)
        ...
        "valid_until": {
            "output": lambda transaction: datetime_to_str_or_none(
        ...
        "payment_method": {
            "expected_input_types": str,
            "output": lambda transaction: spec_payment_method_url(
