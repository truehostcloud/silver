...
from silver.tests.api.specs.document_entry import (
    spec_document_entry,
...
invoice_definition = ResourceDefinition(
    "invoice",
    {
        "id": {
            "read_only": True,
        ...
        "proforma": {
            ...
            "output": lambda invoice: (
                spec_proforma_url(invoice.related_document)
        ...
        "archived_provider": {
            ...
            "output": lambda invoice: (
                ...
                else spec_archived_provider(invoice.provider)
            ),
        },
        "archived_customer": {
            "read_only": True,
            "output": lambda invoice: (
                {}
                if invoice.state == "draft"
                else spec_archived_customer(invoice.customer)
            ),
        },
        "customer": {
            "expected_input_types": str,
            "output": lambda invoice: spec_customer_url(invoice.customer),
            "assertions": [lambda input, invoice, output: input == output],
        },
        "provider": {
            "expected_input_types": str,
        ...
        "transaction_xe_date": {
            ...
            "output": lambda invoice: (
                ...
                if invoice.currency == invoice.transaction_currency
                else date_to_str(invoice.issue_date - timedelta(days=1))
            ),
            "assertions": [
                lambda input, invoice, output: input == output if input else True
            ],
        ...
        "transaction_xe_rate": {
            ...
            "output": lambda invoice: (
                ...
                if invoice.transaction_xe_rate
                else None
            ),
            "assertions": [
                lambda input, invoice, output: (
                    "%.4f" % Decimal(input) == output
                    if input
                    else output is None
                    if invoice.state == "draft"
                    else True  # transaction_xe_rate is based on transaction_xe_date
                )
            ],
        },
        "state": {
        ...
        "total_in_transaction_currency": {
            "read_only": True,
            "output": lambda invoice: decimal_string_or_none(
        ...
        "sales_tax_percent": {
            ...
            "assertions": [
                lambda input, invoice, output: (
                    "%.2f" % Decimal(input) == output
                    if input
                    else output == "%.2f" % invoice.customer.sales_tax_percent
                )
            ],
        },
        "sales_tax_name": {
        ...
        "invoice_entries": {
            ...
            "assertions": [
                lambda input, invoice, output: (
                    len(input) == len(invoice.entries) == len(output)
                    if input
                    else len(invoice.entries) == len(output)
                ),
                # TODO
                # lambda input, invoice, output: (
        ...
        "pdf_url": {
            ...
            "output": lambda invoice: (
                None
                if not (invoice.pdf and invoice.pdf.url)
                else absolute_url(invoice.pdf.url)
            ),
        },
    },
)
