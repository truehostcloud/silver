...
class TestInvoiceGenerationCommand(TestCase):
    ...
    def test_trial_spanning_over_multiple_months(self):
        ...
        metered_feature = MeteredFeatureFactory(
            included_units_during_trial=Decimal("0.00"), price_per_unit=mf_price
        )
        currency = "EUR"
        ...
        # # TEST ##
        call_command(
            "generate_docs", billing_date=prev_billing_date, stdout=self.output
        )
    ...
    def test_gen_consolidated_billing_with_consumed_mfs(self):
        ...
        customer = CustomerFactory.create(
            consolidated_billing=True, sales_tax_percent=Decimal("0.00")
        )
        mf_price = Decimal("2.5")
        metered_feature = MeteredFeatureFactory(
    ...
    def test_prorated_subscription_with_consumed_mfs_overflow(self):
        ...
        mf_price = Decimal("2.5")
        metered_feature = MeteredFeatureFactory(
            included_units=Decimal("20.00"), price_per_unit=mf_price
        )
        plan = PlanFactory.create(
            interval="month",
            interval_count=1,
    ...
    def test_subscription_with_trial_with_metered_features_underflow_to_draft(self):
        ...
        call_command(
            "generate_docs", billing_date=dt.date(2015, 3, 1), stdout=self.output
        )
    ...
    def test_subscription_with_trial_with_metered_features_overflow_to_draft(self):
        ...
        metered_feature = MeteredFeatureFactory(
            included_units=Decimal("0.00"), included_units_during_trial=Decimal("5.00")
        )
        plan = PlanFactory.create(
            interval="month",
            interval_count=1,
    ...
    def test_on_trial_with_consumed_units_overflow(self):
        ...
        metered_feature = MeteredFeatureFactory(
            included_units_during_trial=included_during_trial, price_per_unit=mf_price
        )
        plan = PlanFactory.create(
            interval="month",
            interval_count=1,
    ...
    def test_full_month_with_consumed_units(self):
        ...
        proforma = Proforma.objects.all()[0]
        assert proforma.proforma_entries.all().count() == 2
        assert all([not entry.prorated for entry in proforma.proforma_entries.all()])
    ...
    def test_gen_mixed_states_for_multiple_providers(self):
        ...
        metered_feature = MeteredFeatureFactory(included_units=Decimal("20.00"))
        provider_draft = ProviderFactory.create(default_document_state="draft")
        ...
        mocked_last_billing_date = PropertyMock(return_value=dt.date(2015, 2, 14))
    ...
    def test_cancel_sub_without_trial_at_end_of_billing_cycle(self):
        ...
        metered_feature = MeteredFeatureFactory(
            included_units=Decimal("0.0000"), price_per_unit=Decimal("2.5")
        )
        plan = PlanFactory.create(
            interval=Plan.INTERVALS.MONTH,
            interval_count=1,
        ...
        # RUN 2
        call_command(
            "generate_docs", billing_date=random_billing_date, stdout=self.output
        )
    ...
    def test_gen_for_single_canceled_subscription_during_trial(self):
        ...
        subscription = SubscriptionFactory.create(
            plan=plan, start_date=dt.date(2015, 1, 3)
        )
        subscription.activate()
        ...
        call_command(
            "generate_docs",
            date=generate_docs_date("2015-01-06"),
        ...
        call_command(
            "generate_docs",
            date=generate_docs_date("2015-01-07"),
            subscription=subscription.pk,
            stdout=self.output,
        )
    ...
    def test_gen_active_and_canceled_selection(self):
        ...
        call_command(
            "generate_docs", billing_date=cancel_date + ONE_DAY, stdout=self.output
        )
    ...
    def test_subscription_with_separate_cycles_during_trial(self):
        ...
        metered_feature = MeteredFeatureFactory(
            included_units_during_trial=Decimal("5.00"), price_per_unit=Decimal("1.00")
        )
        plan = PlanFactory.create(
            interval=Plan.INTERVALS.MONTH,
            interval_count=1,
        ...
        subscription = SubscriptionFactory.create(
            plan=plan, start_date=dt.date(2015, 1, 25)
        )
        subscription.activate()
        subscription.save()
        ...
        mf_log = MeteredFeatureUnitsLogFactory.create(
            ...
            end_date=dt.date(2015, 1, 31),
            consumed_units=Decimal("5.00"),
        )

        call_command(
            "generate_docs", date=generate_docs_date("2015-01-25"), stdout=self.output
        )

        assert Proforma.objects.all().count() == 0

        call_command(
            "generate_docs", date=generate_docs_date("2015-02-01"), stdout=self.output
        )
        ...
        call_command(
            "generate_docs",
            date=generate_docs_date("2015-03-01"),
            subscription=subscription.pk,
            stdout=self.output,
        )
        ...
        call_command(
            "generate_docs",
            date=generate_docs_date("2015-02-10"),
            subscription=subscription.pk,
            stdout=self.output,
        )
        assert Proforma.objects.all().count() == 2
    ...
    def test_subscription_with_documents_generation_on_trial_end(self):
        ...
        call_command(
            "generate_docs", date=generate_docs_date("2015-02-09"), stdout=self.output
        )
        proforma = Proforma.objects.all()[1]
        ...
        call_command(
            "generate_docs", date=generate_docs_date("2015-02-10"), stdout=self.output
        )
        assert Proforma.objects.all().count() == 2
