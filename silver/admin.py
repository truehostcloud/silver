...
def tax(obj):
    return (
        "{} {:.2f}%".format(obj.sales_tax_name, obj.sales_tax_percent)
        if obj.sales_tax_percent
        else ""
    )
...
class SubscriptionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "plan": autocomplete.ModelSelect2(url="autocomplete-plan"),
...
class SubscriptionAdmin(ModelAdmin):
    ...
    cancel_at_end_of_cycle.short_description = (
        "Cancel the " "selected Subscription(s) at the end " "of the billing cycle"
    )
...
class CustomerAdmin(LiveModelAdmin):
    ...
    def generate_all_documents(self, request, queryset):
        if request.POST.get("post"):
            ...
            DocumentsGenerator().generate(
                billing_date=billing_date, customers=queryset, force_generate=True
            )
    ...
    generate_all_documents.short_description = (
        "Generate all user's Invoices and Proformas"
    )
...
class ProviderAdmin(LiveModelAdmin):
    ...
    list_display = [
        "name",
        "invoice_series_list_display",
        "proforma_series_list_display",
        metadata,
    ]
    search_fields = [
        "name",
    ...
    def _compute_monthly_totals(self, model_klass, provider, documents):
        ...
        documents_years_months = sorted(
            (year, month) for month, year in documents_months_years if month and year
        )
        ...
        for year_month in documents_years_months:
            if year_month is None:
                continue
            display_date = date(
                day=1, month=year_month[1], year=year_month[0]
            ).strftime("%B %Y")
            ...
            totals[klass_name_plural]["entries"][display_date] = {
                total_key: str(total_value)
                for total_key, total_value in totals_per_date.items()
            }
    ...
    def generate_monthly_totals(self, request, queryset):
        ...
        invoices = Invoice.objects.filter(
            ...
            state__in=[
                BillingDocumentBase.STATES.DRAFT,
                BillingDocumentBase.STATES.ISSUED,
                BillingDocumentBase.STATES.PAID,
            ],
        )
        ...
        for provider in queryset:
            ...
            invoices_total = self._compute_monthly_totals(Invoice, provider, invoices)
            totals[provider.name].update(invoices_total)

            proformas_total = self._compute_monthly_totals(
                Proforma, provider, proformas
            )
...
class DocumentEntryForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {"description": forms.Textarea(attrs={"cols": 50, "rows": 3})}
...
class BillingDocumentForm(forms.ModelForm):
    transaction_currency = ChoiceField(
        choices=(BLANK_CHOICE_DASH + list(currencies)),
        required=False,
    )
...
class InvoiceForm(BillingDocumentForm):
    class Meta:
        ...
        fields = ()
        widgets = {
            "provider": autocomplete.ModelSelect2(url="autocomplete-provider"),
            "customer": autocomplete.ModelSelect2(url="autocomplete-customer"),
        }
...
class BillingDocumentAdmin(ModelAdmin):
    ...
    list_filter = ("provider__company", "state", CustomerFilter, DueDateFilter, "is_storno")
    ...
    customer_search_fields = [
        "customer__{field}".format(field=field)
    ...
    def get_search_results(self, request, queryset, search_term):
        if "-" in search_term and search_term[-1].isdigit():
            return (
                ...
                ).filter(_series_number=search_term),
                True,
            )

        return super(BillingDocumentAdmin, self).get_search_results(
    ...
    def perform_action(
        ...
        readable_past_action = (
            readable_past_action
            if readable_past_action
            else "executed {action}".format(action=action)
        )
        ...
        error_results_count, success_results_count = len(error_results), len(
            success_results
        )
        ...
        if success_results_count:
            documents_pluralization = (
                model_name_plural if success_results_count else model_name
            )

            success_message = (
                "Successfully {readable_past_action} {count} "
                "{documents_pluralization}".format(
                    readable_past_action=readable_past_action,
                    count=success_results_count,
                    documents_pluralization=documents_pluralization,
                )
            )

            success_results_messages = self._parse_results_into_messages(
                success_results
            )
        ...
        if error_results_count:
            documents_pluralization = (
                model_name_plural if error_results_count else model_name
            )

            error_message = (
                "Couldn't {readable_action} {count} {documents_pluralization}".format(
                    readable_action=readable_action,
                    count=error_results_count,
                    documents_pluralization=documents_pluralization,
                )
            )
    ...
    def transactions(self, obj):
        if obj.transaction_xe_rate:
            ...
            return format_html(
                '<a href="{url}" target="_blank">{total:.2f} {currency}</a>'.format(
                    url=url.url,
                    total=obj.total_in_transaction_currency,
        ...
        for document in queryset:
            if document.pdf:
                ...
                try:
                    ...
                    logging_ctx = {"number": document.series_number, "status": "ok"}
...
class InvoiceAdmin(BillingDocumentAdmin):
    ...
    def cancel(self, request, queryset):
        self.perform_action(
            request, queryset, "cancel", readable_past_action="canceled"
        )
    ...
    def create_storno(self, request, queryset):
        self.perform_action(
            ...
            queryset,
            "create_storno",
            readable_action="create storno for",
    ...
    mark_pdf_for_generation.short_description = (
...
class TransactionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "invoice": autocomplete.ModelSelect2(url="autocomplete-invoice"),
            "proforma": autocomplete.ModelSelect2(url="autocomplete-proforma"),
    ...
    def clean(self):
        ...
        if not self.cleaned_data["currency"]:
            self.cleaned_data[
                "amount"
            ] = document.amount_to_be_charged_in_transaction_currency
...
class TransactionAdmin(ModelAdmin):
    ...
    def execute(self, request, queryset):
        ...
        if not failed_count:
            self.message_user(
                request, "Successfully executed %d transactions." % (transactions_count)
            )
        elif failed_count != transactions_count:
            self.message_user(
                request,
                "Executed %d transactions, %d failed." % (settled_count, failed_count),
                level=messages.WARNING,
            )
        else:
            self.message_user(
...
class PaymentMethodAdmin(ModelAdmin):
    list_display = ("customer", "payment_processor", "added_at", "verified", "canceled")
    list_filter = ("customer", "verified", "canceled", "payment_processor")
    search_fields = ["customer__first_name", "customer__last_name", "customer__company"]
