...
class SubscriptionDetail(generics.RetrieveUpdateAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if request.data:
            message = "Cannot update a subscription when it's in %s state." % state
            return Response({"detail": message}, status=status.HTTP_400_BAD_REQUEST)
        request.data.clear()
        request.data.update({"meta": meta} if meta else {})
        return super(SubscriptionDetail, self).patch(request, *args, **kwargs)
...
class MeteredFeatureUnitsLogDetail(APIView):
    ...
    def get(self, request, format=None, **kwargs):
        ...
        metered_feature = get_object_or_404(
            subscription.plan.metered_features, product_code__value=mf_product_code
        )

        logs = MeteredFeatureUnitsLog.objects.filter(
            metered_feature=metered_feature.pk, subscription=subscription_pk
        )

        serializer = MFUnitsLogSerializer(logs, many=True, context={"request": request})
    ...
    def patch(self, request, *args, **kwargs):
        ...
        if not metered_feature:
            return Response(
                {"detail": "Metered Feature Not found."},
        ...
        if subscription.state not in [
            ...
            return Response(
                {"detail": "Subscription is %s." % subscription.state},
        ...
        if date < subscription.start_date:
            return Response(
                {"detail": "Date is out of bounds."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        interval = next(
            (
                i
                for i in subscription.updateable_buckets()
        ...
        if metered_feature not in subscription.plan.metered_features.all():
            err = "The metered feature does not belong to the " "subscription's plan."
        ...
        return Response({"count": log.consumed_units}, status=status.HTTP_200_OK)
