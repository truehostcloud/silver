...
class DocumentEntrySerializer(
    AutoCleanSerializerMixin, serializers.HyperlinkedModelSerializer
):
    product_code = serializers.SlugRelatedField(slug_field="value", read_only=True)
    total_before_tax = DecimalField(
        coerce_to_string=True, max_digits=None, decimal_places=2, read_only=True
    )
    total = DecimalField(
        coerce_to_string=True, max_digits=None, decimal_places=2, read_only=True
    )
...
class DocumentUrl(serializers.HyperlinkedIdentityField):
    ...
    def get_url(self, obj, view_name, request, format):
        view_name = (
            self.invoice_view_name if obj.kind == "invoice" else self.proforma_view_name
        )
        ...
        kwargs = {self.lookup_url_kwarg: str(lookup_value)}
...
class DocumentSerializer(serializers.HyperlinkedModelSerializer):
    ...
    customer = CustomerUrl(view_name="customer-detail", queryset=Customer.objects.all())
