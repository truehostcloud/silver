...
class PaymentMethodUrl(serializers.HyperlinkedRelatedField):
    def get_url(self, obj, view_name, request, format):
        kwargs = {"payment_method_id": obj.pk, "customer_pk": obj.customer_id}
        return self.reverse(view_name, kwargs=kwargs, request=request, format=format)
...
class PaymentMethodSerializer(serializers.HyperlinkedModelSerializer):
    url = PaymentMethodUrl(
        view_name="payment-method-detail", source="*", read_only=True
    )
    ...
    def get_payment_processor(self, obj):
        return PaymentProcessorSerializer(
            obj.get_payment_processor(), context=self.context
        ).data
    ...
    def validate(self, attrs):
        ...
        if self.instance:
            if self.instance.canceled:
                raise ValidationError("You cannot update a canceled payment method.")
