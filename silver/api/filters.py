...
class CustomerFilter(FilterSet):
    ...
    sales_tax_number = CharFilter(
        field_name="sales_tax_number", lookup_expr="icontains"
    )
    consolidated_billing = CharFilter(
        field_name="consolidated_billing", lookup_expr="icontains"
    )
...
class BillingDocumentFilter(FilterSet):
    ...
    customer_company = CharFilter(
        field_name="customer__company", lookup_expr="icontains"
    )
    provider_name = CharFilter(field_name="provider__name", lookup_expr="icontains")
    provider_company = CharFilter(
        field_name="provider__company", lookup_expr="icontains"
    )
...
class InvoiceFilter(BillingDocumentFilter):
    series = CharFilter(field_name="provider__invoice_series", lookup_expr="icontains")

    class Meta(BillingDocumentFilter.Meta):
        model = Invoice
        fields = BillingDocumentFilter.Meta.fields + [
            "series",
        ]


class ProformaFilter(BillingDocumentFilter):
    series = CharFilter(field_name="provider__proforma_series", lookup_expr="icontains")
...
class TransactionFilter(FilterSet):
    payment_processor = CharFilter(
        field_name="payment_method__payment_processor", lookup_expr="iexact"
    )
