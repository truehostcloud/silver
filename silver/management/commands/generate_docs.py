...
def date(date_str):
    try:
        ...
        msg = "Not a valid date: '{date_str}'. " "Expected format: YYYY-MM-DD.".format(
            date_str=date_str
        )
    ...
    def handle(self, *args, **options):
        ...
        if options["subscription_id"]:
            try:
                ...
                logger.info(
                    "Generating for subscription with id=%s; " "billing_date=%s.",
                    subscription_id,
                    billing_date,
                )
                ...
                docs_generator.generate(
                    subscription=subscription, billing_date=billing_date
                )
            ...
            logger.info(
                "Generating for all the available subscriptions; " "billing_date=%s.",
                billing_date,
            )
