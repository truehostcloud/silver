...
def field_template_path(field, provider=None):
    if provider:
        provider_template_path = "billing_documents/{provider}/{field}.html".format(
...
class MeteredFeatureUnitsLog(models.Model):
    metered_feature = models.ForeignKey(
        "MeteredFeature", related_name="consumed", on_delete=models.CASCADE
    )
    subscription = models.ForeignKey(
        "Subscription", related_name="mf_log_entries", on_delete=models.CASCADE
    )
    ...
    class Meta:
        unique_together = ("metered_feature", "subscription", "start_date", "end_date")

    def clean(self):
        super(MeteredFeatureUnitsLog, self).clean()
        if self.subscription.state in [
...
class Subscription(models.Model):
    ...
    def _get_last_start_date_within_range(
        ...
        relative_start_date = (
            range_start if aligned_start_date > range_end else aligned_start_date
        )
    ...
    def _cycle_end_date(self, reference_date=None, ignore_trial=None, granulate=None):
        ...
        real_cycle_start_date = self._cycle_start_date(
            reference_date, ignore_trial, granulate
        )
    ...
    def should_be_billed(self, billing_date, generate_documents_datetime=None):
        ...
        cycle_start_datetime = datetime.combine(
            cycle_start_date, datetime.min.time()
        ).replace(tzinfo=utc)
    ...
    ##########################################################################
    @transition(
        field=state, source=[STATES.INACTIVE, STATES.CANCELED], target=STATES.ACTIVE
    )
    ...
    def _add_trial_value(self, start_date, end_date, invoice=None, proforma=None):
        self._add_plan_trial(
            start_date=start_date, end_date=end_date, invoice=invoice, proforma=proforma
        )
        self._add_mfs_for_trial(
            start_date=start_date, end_date=end_date, invoice=invoice, proforma=proforma
        )
    ...
    def _log_value_state(self, value_state):
        logger.debug(
            "Adding value: %s", {"subscription": self.id, "value_state": value_state}
        )
    ...
    def _add_mfs_for_trial(self, start_date, end_date, invoice=None, proforma=None):
        ...
        for metered_feature in self.plan.metered_features.all():
            ...
            if charged_units > 0:
                context.update({"context": "metered-feature-trial-not-discounted"})
    ...
    def _add_plan_value(self, start_date, end_date, invoice=None, proforma=None):
        """
    ...
    def _get_proration_status_and_percent(start_date, end_date):
        ...
        last_day_index = calendar.monthrange(start_date.year, start_date.month)[1]
        last_day_of_month = date(start_date.year, start_date.month, last_day_index)
    ...
    def __str__(self):
        return "%s (%s)" % (self.customer, self.plan.name)


class BillingLog(models.Model):
    subscription = models.ForeignKey(
        "Subscription", on_delete=models.CASCADE, related_name="billing_logs"
    )
