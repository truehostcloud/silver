...
class DocumentsGenerator(object):
    def generate(
        ...
        if not subscription:
            ...
            self._generate_for_single_subscription(
    ...
    def _generate_for_user_with_consolidated_billing(
        ...
        for subscription in self.get_subscriptions_prepared_for_billing(
            ...
            existing_provider_documents[
                provider
            ] = self._bill_subscription_into_document(
    ...
    def add_subscription_cycles_to_document(
        ...
        while relative_start_date <= last_cycle_end_date:
            ...
            # billing date.
            should_bill_plan = (
                (plan_billed_up_to < relative_start_date)
                if prebill_plan
                else (relative_end_date < billing_date)
            )

            # Bill the plan amount
            if should_bill_plan:
                if subscription.on_trial(relative_start_date):
                    plan_amount += subscription._add_plan_trial(
                    ...
                    plan_amount += subscription._add_plan_value(
                        start_date=relative_start_date,
                        end_date=relative_end_date,
                        proforma=proforma,
                        invoice=invoice,
    ...
    def _create_document(self, subscription, billing_date):
        ...
        DocumentModel = (
            Proforma if provider.flow == provider.FLOWS.PROFORMA else Invoice
        )
